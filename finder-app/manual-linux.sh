...
PATH_LIB=/usr/local/arm-cross-compiler/install/gcc-arm-10.2-2020.11-x86_64-aarch64-none-linux-gnu/aarch64-none-linux-gnu/libc/
...
${CROSS_COMPILE}readelf -a bin/busybox | grep "program interpreter" | cut -d':' -f2 | tr -d ' ]' | xargs -i cp ${PATH_LIB}{} lib
${CROSS_COMPILE}readelf -a bin/busybox | grep "Shared library" | cut -d'[' -f2 | tr -d ' ]' | xargs -i cp ${PATH_LIB}/lib64/{} lib64
